...
const DashboardSearch = () => {
  ...
  const getSearchResults = async () =>
    await api.getSearchResults(query.get("search")).then((response) => {
      let arr = response.data;
      console.log(response.data);
      arr.map((productCat) => {
        productCat.products.map((products) => {
          products.product_variations = products.product_variations.map(
            (variations) => {
  ...
  const getSupplierSearchResults = async () =>
    await api
      .getSupplierSearchResults(query.get("search"), query.get("supplier"))
      .then((response) => {
        let arr = response.data;
        arr.map((productCat) => {
          productCat.products.map((products) => {
        ...
        <>
          ...
          <div className="flex justify-between">
            <TitleUnderLine
              ...
              nounderline={true}
              title={`Search results for "${query.get("search")}"`}
            />
          </div>
          {searchData.length > 0 &&
            searchData.map((productCat) => {
              return productCat.products.map((product) => {
                return (
                  <div key={uuid()}>
