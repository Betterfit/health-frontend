...
const DashboardMatches = () => {
  ...
  const sortFunction = () => {
    if (matchStore.matches) {
      ...
      api.postSortedMatches(arr).then((response) => {
        let arr = response.data;
        arr = arr.matches.map((item) => {
          let obj = {
            order_no: item.order_no,
  ...
  const HeadingComponent = ({ title, value, classes, time }) => {
    if (time) {
      ...
      return (
        ...
        >
          <span className="uppercase betterfit-graphite text-xxs tracking-extra-wide opacity-50 font-semibold">
            {title}
          </span>
              ...
              <div className="flex flex-row mt-4 items-center">
                ...
                >
                  {refresh ? (
                    <ReactSVG
                      src={Reload}
                      className={`flex items-center mr-3 rotate-me`}
                      beforeInjection={(svg) => {
