...
const DashboardMatchesOrderDetail = (props) => {
    ...
    await api
      ...
      .then((response) => {
        ...
        arr = arr.map((item) => {
          ...
          let obj = {
            ...
            item:
              ...
              " - " +
              item.product_option.product_variation,
        ...
        setOrderHeader({
          ...
          match_date:
            data.match_date !== "No match date"
              ? dayjs(data.order_date).format("MMM d, YYYY")
              : "No match date",
          ordered_by: data.facility.name,
          facility: data.facility.name,
          address: data.facility.street
            ? `${data.facility.street}, ${data.facility.city}, ${data.facility.postal_code}`
            : null,
          shipping_address: data.facility.shipping_street
            ? `${data.facility.shipping_street}, ${data.facility.shipping_city}, ${data.facility.shipping_postal_code}`
            : null,
        });
      })
  ...
  const actionComponent = (
    <StatusButton
      status={`${
        orderHeader.match_date !== "No match date" ? "matched" : "no-match"
      }`}
    />
  );
