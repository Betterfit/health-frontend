...
const Search = ({ type, activeTab = null }) => {
  ...
  const searchQuery = () => {
    ...
    if (activeTab) {
      if (activeTab == "my-inventory") {
        ...
        history.push(
          `/dashboard/inventory/search?search=${searchRef.current.value}`
        );
      }
    }
  };
  ...
  if (type === "icon") {
    return (
      <div
        className={`flex items-center pt-2 h-full bg-betterfit-pale-blue ${
          showInput ? "absolute w-full z-10 " : "relative"
        }`}
      >
        <button
          className="button-reset px-4"
    ...
    return (
      <div
        className={`flex items-center h-full bg-betterfit-pale-blue items-center px-6 py-1`}
        style={{ borderRadius: 30 }}
      >
        <ReactSVG className="ml-2 mr-2" src={SearchIcon} />
        <div className="relative flex-grow">
