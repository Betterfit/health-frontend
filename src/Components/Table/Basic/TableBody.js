        ...
        if (index % 2 === 0) {
          return (
            <tr key={uuid()} className="bg-gray-100 border-t border-gray-200">
              ...
              <td
                key={uuid()}
                className="px-4 py-4 w-8 text-sm leading-5 text-betterfit-graphite"
              >
                <NavLink
                  to={`/dashboard/inventory/product/${variantID}/detail/${
                    edit ? (NoOptions ? "edit" : setIndex + "/edit") : setIndex
                  }`}
                >
                  {edit ? (
                    <ReactSVG
