...
const MatchOrderDetailHeader = ({ order, actionComponent }) => {
  ...
  return (
    <div className="flex flex-col border-b pb-4 border-gray-400 relative">
      ...
      <MatchHeaderComponentTitle value={`#${order.order_number}`} />
      <div className="flex-col flex-row mt-4 md:w-3/4">
        <div className="flex-row flex mb-12 justify-between">
          ...
          />
          {order.match_date && (
            <MatchHeaderComponent title="Match Date" value={order.match_date} />
          )}
          <MatchHeaderComponent title="Ordered by" value={order.facility} />
        </div>
        <div className="flex-row flex justify-between">
          {order.address && (
            <MatchHeaderComponent title="Address" value={order.address} />
          )}
          {order.address && (
            <MatchHeaderComponent
