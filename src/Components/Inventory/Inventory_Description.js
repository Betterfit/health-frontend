...
function Inventory_Description({ title, description, class_addons }) {
  ...
  return (
    <dl>
      <dt className={`${standard_dt_styles} font-semibold text-10 tracking-extra-wide text-gray-600 opacity-50`}>{Translator(title)}</dt>
      <dd className="text-betterfit-grey-blue text-base leading-6 pb-3">{description}</dd>
    </dl>
  );
}
